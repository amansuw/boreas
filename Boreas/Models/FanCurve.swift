...
struct CurvePoint: Identifiable, Codable, Equatable {
    let id: UUID
    var temperature: Double
    var fanSpeed: Double // percentage 0-100
...
struct FanCurve: Identifiable, Codable, Equatable {
    let id: UUID
    var name: String
    var points: [CurvePoint]
    var sensorKey: String // which temperature sensor to use
    ...
    mutating func removePoint(at index: Int) {
        guard points.count > 2 else { return } // Keep at least 2 points
    ...
    mutating func updatePoint(id: UUID, temperature: Double? = nil, fanSpeed: Double? = nil) {
        if let index = points.firstIndex(where: { $0.id == id }) {
            if let temp = temperature {
                points[index].temperature = temp
            }
            if let speed = fanSpeed {
                points[index].fanSpeed = speed
            }
        }
    }
}
